...
FFMPEG *ffmpeg_start_rendering(size_t width, size_t height, size_t fps, const char * file_name)
{
    ...
    snprintf(cmd_buffer, sizeof(cmd_buffer), "ffmpeg.exe -loglevel verbose -y -f rawvideo -pix_fmt rgba -s %dx%d -r %d -i  - -c:v libx264 -vb 2500k -c:a aac -ab 200k -pix_fmt yuv420p %s", (int)width, (int)height, (int)fps , file_name);
